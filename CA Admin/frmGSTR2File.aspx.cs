    ...
    protected void btnGetSummary_Click(object sender, EventArgs e)
    {
        ...
        if (dt != null)
        {
            if (dt.Rows[0]["res"].ToString() == "1")
            {
                ...
                GSTR2Summ objGstr2summ = JsonConvert.DeserializeObject<GSTR2Summ>(dt.Rows[0]["Summary"].ToString());
                GridGSTR2Summ.DataSource = objGstr2summ.section_summary;
                GridGSTR2Summ.DataBind();
    ...
    protected void btnFiled_Click(object sender, EventArgs e)
    {
        if (ViewState["Summary"] != null)
        {
           ...
           objpl.SummaryData = ViewState["Summary"].ToString();
            ...
            lblUploadStatus.Text = "";
            btnRefresh.Visible = false;
            DataTable dt = CLSCommon.CallApiPost("api/GSTR2File/FileGSTR2", objpl);
            if (dt != null)
            {
                if (dt.Rows[0]["res"].ToString() == "1")
                {
